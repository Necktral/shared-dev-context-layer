...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `TD-001` | composición bootstrap | descomposición aplicada: `vscode-extension/src/extension.ts` quedó en `28 LOC` y el wiring se movió a módulos de activación/registradores | riesgo lateral por concentración mitigado; composición explícita por frontera de comando | `high` | maintainers de extensión/control-plane | evidencia: `wc -l vscode-extension/src/extension.ts` <= 450 + `run_contract_closure` (`extension`, `local-db`, `backend`) en verde | `closed` |
